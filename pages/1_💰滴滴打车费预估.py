...
# ========== 基础初始化工作 ==========
# 日志级别设置
logging.basicConfig(level=logging.INFO)  # 如需要更细致的观察run状态时可以将 `level` 的值改为 `logging.DEBUG`
...
if prompt := st.chat_input():
    ...
    while True:
        try:
            ...
            run = client.beta.threads.runs.cancel(thread_id=st.session_state.thread.id, run_id=run.id)
            logging.debug(run)
            break
