...
export function AuthDialog({ trigger, defaultForm = "login", onSuccess }: AuthDialogProps) {
  ...
  const renderForm = () => {
    switch (currentForm) {
      ...
      case "feedback":
        return <FeedbackForm isDialog onSuccess={handleSuccess} onLoginClick={() => setCurrentForm("login")} />
      default:
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px] md:max-w-[500px] lg:max-w-[550px]">{renderForm()}</DialogContent>
    </Dialog>
  )
}
