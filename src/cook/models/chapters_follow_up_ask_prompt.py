from sqlalchemy import create_engine, insert, select, Column, Integer, String, Text
...
def update_follow_up_ask_prompt_template(lark_app_token, lark_table_id, prompt_template):
    ...
    try:
        # 首先尝试更新现有记录
        stmt = update(ChaptersFollowUpAskPrompt).where(
...
def get_follow_up_ask_prompt_template(lark_table_id):
    session = Session()
    try:
        result = session.query(ChaptersFollowUpAskPrompt.prompt_template).filter(
            ChaptersFollowUpAskPrompt.lark_table_id == lark_table_id
        ).first()
...
def update_ask_info_from_api(lesson_id, lesson_ask_prompt, lesson_summary,
                    lesson_ask_model='qwen2-72b-instruct', lesson_ask_count_history=5, lesson_ask_count_limit=5,
