...
from tools.umami_event_analysis import get_trail_script_count, get_chapter_visit_user_by_start_with, \
...
with login():
    ...
    with tab2:
        ...
        chapter_from = []
        for i in range(8):
            chapter_from.append(get_chapter_visit_user_by_start_with(f'{i:02}', 'from')['string_value'].to_list())

        chapter_arrivals = []
        for i in range(8):
            chapter_arrivals.append(get_chapter_visit_user_by_start_with(f'{i:02}')['string_value'].to_list())
        ...
        # 绘制漏斗图
        fig = px.funnel(df, x='到达人数', y='章节名称', title=f'章节漏斗 ({current_time})', height=script_num*60)
        st.plotly_chart(fig, use_container_width=True)
        ...
        # 绘制图表
        fig = px.line(df, x='章节名称', y='下降比例', title=f'章节间下降比例 ({current_time})',
