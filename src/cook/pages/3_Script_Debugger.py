...
if st.session_state['authentication_status']:
    ...
    with col1:
        if st.button('刷新整个课程', use_container_width=True):
            st.cache_data.clear()
    with col2:
        if st.button(f'加载指定的单条剧本', type='primary', use_container_width=True):
            # 加载剧本及系统角色
    ...
    if st.session_state.is_single_script_loaded:
        ...
        with col2:
            ...
            needed_vars = extract_variables(st.session_state.debug_script.template) + system_role_needed_vars
        ...
        if st.session_state.debug_script.type == ScriptType.FIXED and st.session_state.debug_script.check_template == '未填写！':
            ...
            col1, col2= st.columns(2)
            with col1:
                test_times = st.number_input('列数(一个模型测几遍)：', value=3, min_value=1, step=1)
            with col2:
                max_height = st.number_input('最大行高：', value=300, min_value=100, step=100)
