...
def create_active (app, course_id,active_name,active_desc
                   ,active_start_time,active_end_time,active_price):
    active = Active()
    active.active_id = generate_id(app)
    active.active_name = active_name
    active.active_desc = active_desc
    # active.active_type = active_type
    ...
    # active.active_discount_type = active_discount_type
    # active.active_discount_desc = active_discount_desc
...
def create_active_user_record (app, active_id,user_id,price,order_id,status,active_name)->ActiveUserRecord:
...
def query_and_join_active(app,course_id,user_id,order_id)->list[ActiveUserRecord]:
    ...
    print(now)
    active_infos = Active.query.filter(Active.active_course == course_id,
                                       Active.active_status == 1,
                                       Active.active_start_time<= now,
                                       Active.active_end_time >= now
                                       ).all()
    if not active_infos:
        app.logger.info('no active for course:{} and user:{}'.format(course_id,user_id))
        return []
    active_user_records = []
    for active_info in active_infos:
        app.logger.info('active info:{}'.format(active_info.active_name))
        active_user_record = ActiveUserRecord.query.filter(ActiveUserRecord.active_id == active_info.active_id,ActiveUserRecord.user_id == user_id).first()
        if active_user_record:
            active_user_records.append(active_user_record)
        else:
            active_user_records.append(create_active_user_record(app,active_info.active_id,user_id,active_info.active_price,order_id,0,active_info.active_name))
