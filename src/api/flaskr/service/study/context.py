...
from flaskr.service.study.plugin import handle_shifu_input, handle_shifu_output
import queue
...
class RunScriptContext:
    ...
    def __init__(
        ...
        self._trace_args = {}
        self._trace_args["user_id"] = user_id
    ...
    def _get_current_attend(self) -> AICourseLessonAttend:
        attend_info: AICourseLessonAttend = (
            AICourseLessonAttend.query.filter(
                ...
                AICourseLessonAttend.user_id == self._user_id,
    ...
    def run(self, app: Flask) -> Generator[str, None, None]:
        ...
        if self._input_type not in ["start", "continue"]:
            error_result_resp = handle_shifu_input(
                app=app,
            ...
            if error_result_resp:
                yield from error_result_resp
                self._can_continue = True
                return
        else:
            error_result_resp = handle_shifu_output(
                app=app,
                user_id=self._user_id,
                attend_id=self._current_attend.attend_id,
                outline_item_info=self._outline_item_info,
