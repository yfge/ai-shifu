...
def get_learn_record(
    ...
    with app.app_context():
        ...
        # Keep the latest record per (generated_block_bid, position).
        latest_audio_by_block_pos: dict[tuple[str, int], LearnGeneratedAudio] = {}
        for record in audio_records:
            pos = int(getattr(record, "position", 0) or 0)
            key = (record.generated_block_bid, pos)
            if key not in latest_audio_by_block_pos:
                latest_audio_by_block_pos[key] = record

        audio_parts_by_block: dict[str, list[LearnGeneratedAudio]] = {}
        for (block_bid, _pos), record in latest_audio_by_block_pos.items():
            audio_parts_by_block.setdefault(block_bid, []).append(record)

        for parts in audio_parts_by_block.values():
            parts.sort(key=lambda r: int(getattr(r, "position", 0) or 0))
        ...
        for generated_block in generated_blocks:
            ...
            audio_parts = audio_parts_by_block.get(generated_block.generated_block_bid)
            audio_list = (
                [
                    GeneratedAudioPartDTO(
                        position=int(getattr(p, "position", 0) or 0),
                        audio_url=p.oss_url or "",
                        audio_bid=p.audio_bid or "",
                        duration_ms=int(p.duration_ms or 0),
                    )
                    for p in (audio_parts or [])
                    if p and p.oss_url
                ]
                if audio_parts
                else None
            )
            audio_url = audio_list[0].audio_url if audio_list else None
...
def stream_generated_block_audio(
    ...
    *,
    ...
    with app.app_context():
        ...
        # Split by sandbox boundaries (e.g. SVG/mermaid/images). Each speakable
        # section becomes an audio part with a 0-based `position`.
        raw_parts = split_text_by_sandbox_boundaries(raw_text)
        speakable_parts: list[tuple[str, str]] = []
        for part in raw_parts:
            cleaned_part = preprocess_for_tts(part or "")
            if cleaned_part and len(cleaned_part.strip()) >= 2:
                speakable_parts.append((part or "", cleaned_part))
        ...
        expected_part_count = len(speakable_parts)

        # Load existing completed audios once. When expected_part_count > 1, we
        # only reuse "segmented" audio sets (i.e. any record with position > 0).
        existing_records = (
            LearnGeneratedAudio.query.filter(
        ...
        latest_by_position: dict[int, LearnGeneratedAudio] = {}
        has_position_gt0 = False
        for record in existing_records:
            pos = int(getattr(record, "position", 0) or 0)
            if pos > 0:
                has_position_gt0 = True
            if pos not in latest_by_position:
                latest_by_position[pos] = record

        # Single-part blocks can reuse the existing position=0 audio.
        if expected_part_count == 1:
            existing_audio = latest_by_position.get(0)
        ...
        try:
            for position, (part_text, cleaned_part_text) in enumerate(speakable_parts):
                is_last = position == expected_part_count - 1

                # When there are multiple parts, only reuse already-segmented sets.
                if expected_part_count > 1 and has_position_gt0:
                    existing = latest_by_position.get(position)
                    if existing and existing.oss_url:
                        yield RunMarkdownFlowDTO(
                            outline_bid=generated_block.outline_item_bid or "",
                            generated_block_bid=generated_block_bid,
                            type=GeneratedType.AUDIO_COMPLETE,
                            content=AudioCompleteDTO(
                                position=position,
                                audio_url=existing.oss_url,
                                audio_bid=existing.audio_bid,
                ...
                audio_record = LearnGeneratedAudio(
                    ...
                    model=tts_model or "",
                    text_length=len(cleaned_part_text or ""),
                ...
                raw_length = len(part_text or "")
                cleaned_length = len(cleaned_part_text or "")
                record_tts_usage(
                    app,
                    usage_context,
