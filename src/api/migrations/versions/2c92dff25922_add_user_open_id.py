...
def upgrade():
    ...
    with op.batch_alter_table('user_info', schema=None) as batch_op:
        batch_op.add_column(sa.Column('user_open_id', sa.String(length=255), nullable=True, comment='user open id'))
        batch_op.create_index(batch_op.f('ix_user_info_user_open_id'), ['user_open_id'], unique=False)
